...
def median(cam_ID, length, nth_frame, width_trim, height_trim, path):
    """ (str, int, int, int, str)
    ...
    for counter in range(length):
        ...
        if counter % nth_frame == 0:
            ...
            frameMedian.append(image)

    median = np.median(frameMedian, axis=0).astype(dtype=np.uint8)
    cv2.imshow('Calculated Median', median)

    # background = np.percentile(frameMedian, 90, axis=0).astype(dtype=np.uint8)
    # cv2.imshow('Calculated background', background)

    date = datetime.datetime.now().strftime("%Y%m%d")
    cv2.imwrite(path + "/" + date + '_Median.png', median)
